...
#                    raw activity, excess (unsupported) activity, and Φ.                   #
#                                                                                                 #
#    INPUTS: df with col names 'Z_midpt (cm)', 'C_i at collection (dpm/g)',                      #
#    'salt/mud corrected activity (dpm/g)', 'ΔZ (cm)',                           #
#    'Error_MudSaltCorr (Xdir_error)'.                                                            #
###                                                                                             ###
###################################################################################################
def pb_quickplot(cts, mindepth_cm, maxdepth_cm):
    import matplotlib.pyplot as plt
    import pandas as pd
    import numpy as np
    import math
    ...
    print(
        "|-----------------------  PB_QUICKPLOT STARTED  -------------------------|"
    )
    print(
        f"|------------- mindepth (cm): {mindepth_cm} -------- maxdepth (cm): {maxdepth_cm} -------------|"
    )
    print("    ")

    # Define a 3-subplot figure
    fig, (ax1, ax2, ax3) = plt.subplots(
        1, 3, gridspec_kw={"width_ratios": [5, 5, 1]}, sharex=False, sharey=True
    )
    ...
    ax1.errorbar(
        ...
        cts["Z_i (cm)"],
        yerr=cts["ΔZ (cm)"] / 2,
        xerr=cts["Error_MudSaltCorr (Xdir_error)"],
        fmt="none",
        color="0.5",
    )
    ax1.grid(visible=True, which="both", axis="both", color="0.9")
    xUpperBound = cts["C_i at collection (dpm/g)"].max() + 1
    xLowerBound = cts["C_i at collection (dpm/g)"].min() - 0.1
    ...
    ax2.errorbar(
        cts["salt/mud corrected activity (dpm/g)"],
        cts["Z_midpt (cm)"],
        yerr=cts["ΔZ (cm)"] / 2,
        xerr=cts["Error_MudSaltCorr (Xdir_error)"],
        fmt="none",
    ...
    ax2.set_ylim([mindepth_cm, maxdepth_cm])
    ax2.invert_xaxis()
    ax2.invert_yaxis()
    ax2.set_xscale("log")
    ...
    ax3.plot(cts["Φ (Salt corr.)"], cts["Z_midpt (cm)"], color="k")
    ax3.set_xlabel("Salt Corr. Φ")
...
def printSlope(cts):
    ...
    popt, pcov = curve_fit(
        func, cts["salt/mud corrected activity (dpm/g)"], cts["Z_midpt (cm)"]
    )
    ax2.plot(
        cts["salt/mud corrected activity (dpm/g)"],
        func(cts["salt/mud corrected activity (dpm/g)"], *popt),
        color="k",
    )
